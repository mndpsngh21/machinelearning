...
# we are going to try kernel='sigmoid' and random_state=0
...
# expected result from is [[54 14]
#                          [12 20]  ]
# which means only 12+14 are wrong predictions
# oops this has wrost out than other SVM machines, it may work good in some cases
...
plt.title('SVM Sigmoid Regression (Training set)')
plt.xlabel('Age')
...
for i, j in enumerate(np.unique(y_set)):
    plt.scatter(X_set[y_set == j, 0], X_set[y_set == j, 1],
                c = ListedColormap(('red', 'green'))(i), label = j)
plt.title('SVM Sigmoid Regression (Test set)')
plt.xlabel('Age')
plt.ylabel('Estimated Salary')
plt.legend()
plt.show()